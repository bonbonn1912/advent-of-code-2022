...
const computeSizes = () =>
  nodes.filter((n) => n.type == "dir").map((n) => (n.size = dirSize(n.id)));
const createNode = (params) => nodes.push({ ...params, id: nodes.length }) - 1;
const dirSize = (id) =>
  nodes[id].size ||
  nodes
    .filter((n) => n.parentId == id)
    .reduce((a, n) => (n.type == "file" ? a + n.size : a + dirSize(n.id)), 0);

const createNodeIfNotExists = (name, parentId) => {
  let tmp = nodes.filter(
  ...
  readTXTContent("input.txt").trimEnd().replace(/[\r]/g, "").split("\n").map((line) => {
    ...
    if (cmd[0] == "$") {
      if (cmd[1] == "cd") {
        ...
        else ptr = createNodeIfNotExists(cmd[2], ptr); 
      }
    } else {
      if (cmd[0] == "dir") createNodeIfNotExists(cmd[0], ptr);
      else {
        createNode({
          type: "file",
          name: cmd[1],
...
console.log(
  "Part 1 : " +
  nodes
    .filter((n) => n.type == "dir" && n.size <= 100000)
    .reduce((a, n) => a + n.size, 0)
);
console.log(
  "Part 2 : " +
  nodes
    .filter(
      (n) => n.type == "dir" && 70000000 - nodes[0].size + n.size >= 30000000
    )
    .sort((a, b) => a.size - b.size)[0].size
);
/** TODO: DSA lernen Hilfe */